...
if (isset($_REQUEST['sector'])) {
	...
	$db->query('SELECT * FROM ' . $uni . '_Buildings WHERE sector = \'' . $sector . '\'');
	...
	<head>
		...
		<script language="Javascript">
			...
			function updateResources(key,reset) {
				...
				var params = "uni=" + <?php echo '"' . $uni . '"'; ?> + "&sector=" + <?php echo '"' . $sector . '"'; ?> + "&resource=" + resource + "&sort=" + sort_var + "&order=" + sort_order;
				...
				xmlhttp.onreadystatechange = function () {
					if (xmlhttp.readyState == 4) {
						...
						document.getElementById("resource_body").innerHTML = "<img src=\"http://mapper.pardus-alliance.com/images/ajax-loader.gif\" />";
					}
				}
				xmlhttp.send(params);
			}
				...
				<table>
					...
					<?php
						if (isset($_SESSION['id']) && $_SESSION['id'] > 0) {
							echo '<tr><td><a href="' . $base_url . '/' . $uni . '/' . $sector . '/resources/' . $_SESSION['user'] . '">Personal Resource List</a></td></tr>';
						}
					?>
